...
class Roleadmin_model extends CI_Model
{
    ...
    public function roleadmin_show($type='', $page='', $per_page='', $q=array())
	{
        ...
        if(!empty($q)){
            ...
            if(!empty($q['s_fristname'])){
                $query = $this->zerowaste_db->like('deqp_com_user.firstname', $q['s_fristname'], 'both');
            }
            if(!empty($q['s_lastname'])){
                $query = $this->zerowaste_db->like('deqp_com_user.lastname', $q['s_lastname'], 'both');
            }
            if(!empty($q['s_office'])){ 
                ///
            }
            
            if(!empty($q['s_admin_role_code'])){
                $query = $this->zerowaste_db->where('zrw_admin_profile.admin_role_code', $q['s_admin_role_code']);
            }
            if(!empty($q['s_wait_confirm'])){
                $query = $this->zerowaste_db->where('zrw_admin_profile.zrw_admin_id IS NULL',null);  
            }
        }       
        ...
        if(!empty($q['s_prov_code'])){
            $query = $this->zerowaste_db->where('zrw_admin_province.prov_code', $q['s_prov_code']);
            // $query = $this->zerowaste_db->join('zrw_admin_province', 'zrw_admin_profile.zrw_admin_id = zrw_admin_province.zrw_admin_id AND zrw_admin_province.prov_code = '.$q['s_prov_code'].' ', 'inner');
        }   
